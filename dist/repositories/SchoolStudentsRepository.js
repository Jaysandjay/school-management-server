...
exports.SchoolStudentsRepository = void 0;
...
class SchoolStudentsRepository {
    ...
    async addStudent(studentInfo) {
        const client = await pool.connect();
        try {
            const res = await client.query('INSERT INTO students(first_name, last_name, date_of_birth, grade_level) VALUES($1, $2,  $3, $4) RETURNING *', [studentInfo.firstName, studentInfo.lastName, studentInfo.dateOfBirth, studentInfo.gradeLevel]);
    ...
    async updateStudent(studentId, studentInfo) {
        const client = await pool.connect();
        try {
            const res = await client.query(`
                ...
                SET 
                    ...
                    date_of_birth=$3, 
                    grade_level=$4 
                WHERE student_id=$5`, [studentInfo.firstName, studentInfo.lastName, studentInfo.dateOfBirth, studentInfo.gradeLevel, studentId]);
        }
        catch (err) {
    ...
    async deleteStudent(studentId) {
        const client = await pool.connect();
        try {
            const res = await client.query('DELETE FROM students WHERE student_id = $1', [studentId]);
            if (res.rowCount === 0) {
    ...
    async assignStudentGuardian(studentId, guardianId, relationship) {
        const client = await pool.connect();
        try {
            const res = await client.query('INSERT INTO student_guardians(student_id, guardian_id, relationship) VALUES($1, $2, $3)', [studentId, guardianId, relationship]);
    ...
    async deleteStudentGuardian(studentId, guardianId) {
        const client = await pool.connect();
        try {
            const res = await client.query(`DELETE FROM student_guardians WHERE student_id=$1 AND guardian_id=$2`, [studentId, guardianId]);
    ...
    async getStudentAvailableGuardians(studentId) {
        const client = await pool.connect();
        try {
            const guardians = await client.query(`SELECT * FROM guardians`);
            const assignedGuardians = await client.query(`SELECT guardian_id FROM student_guardians WHERE student_id=$1`, [studentId]);
            const guardianIds = assignedGuardians.rows.map(row => row.guardian_id);
    ...
    async addStudentAddress(studentId, address) {
        const client = await pool.connect();
        try {
    ...
    async updateStudentAddress(studentId, address) {
        const client = await pool.connect();
        try {
            const res = await client.query(`
                UPDATE addresses a 
    ...
    async getStudentsClasses(studentId) {
        const client = await pool.connect();
        try {
            const enrollments = await client.query(`SELECT class_id FROM enrollments WHERE student_id=$1`, [studentId]);
            const classIds = enrollments.rows.map(row => row.class_id);
            const studentClasses = await client.query(`SELECT * from classes WHERE class_id = ANY($1)`, [classIds]);
    ...
    async getAvailableStudentClasses(studentId) {
        const client = await pool.connect();
        try {
            const enrollments = await client.query(`SELECT class_id FROM enrollments WHERE student_id=$1`, [studentId]);
            const classIds = enrollments.rows.map(row => row.class_id);
            const availableClasses = await client.query(`SELECT * from classes WHERE class_id <> ALL($1)`, [classIds]);
    ...
    async enrollStudent(studentId, classId) {
        const client = await pool.connect();
        try {
            const res = await client.query(`INSERT INTO enrollments(student_id, class_id) VALUES($1, $2)`, [studentId, classId]);
    ...
    async unenrollStudent(studentId, classId) {
        const client = await pool.connect();
        try {
            const res = await client.query('DELETE FROM enrollments WHERE student_id=$1 AND class_id=$2', [studentId, classId]);
            if (res.rowCount === 0) {
    ...
    async updateStudentGrade(studentId, classId, grade) {
        const client = await pool.connect();
        try {
            const res = await client.query(`
                UPDATE enrollments SET grade=$1 WHERE student_id=$2 AND class_id=$3 
                `, [grade, studentId, classId]);
            const enrollment = await client.query(`SELECT * FROM enrollments WHERE student_id=$1 AND class_id=$2`, [studentId, classId]);
...
exports.SchoolStudentsRepository = SchoolStudentsRepository;
