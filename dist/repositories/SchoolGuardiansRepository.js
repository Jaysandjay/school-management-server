...
class SchoolGuardiansRepository {
    ...
    async getGuardian(guardianId) {
        const client = await pool.connect();
        try {
            const res = await client.query('SELECT * FROM guardians WHERE guardian_id = $1', [guardianId]);
            if (res.rowCount === 0) {
    ...
    async addGuardian(guardianInfo) {
        const client = await pool.connect();
        try {
            const res = await client.query('INSERT INTO guardians(first_name, last_name, phone, email) VALUES($1, $2,  $3, $4)', [guardianInfo.firstName, guardianInfo.lastName, guardianInfo.phone, guardianInfo.email]);
    ...
    async updateGuardian(guardianId, updatedGuardian) {
        const client = await pool.connect();
        try {
            const res = await client.query(`UPDATE guardians SET first_name=$1, last_name=$2, phone=$3, email=$4 WHERE guardian_id=$5`, [updatedGuardian.firstName, updatedGuardian.lastName, updatedGuardian.phone, updatedGuardian.email, guardianId]);
        }
        catch (err) {
    ...
    async deleteGuardian(guardianId) {
        const client = await pool.connect();
        try {
            const res = await client.query('DELETE FROM guardians WHERE guardian_id = $1', [guardianId]);
            if (res.rowCount === 0) {
    ...
    async getAvailableGuardianStudents(guardianId) {
        const client = await pool.connect();
        try {
            const students = await client.query(`SELECT * FROM students`);
            const assignedStudents = await client.query(`SELECT student_id FROM student_guardians WHERE guardian_id=$1`, [guardianId]);
            const studentIds = assignedStudents.rows.map(row => row.student_id);
    ...
    async getGuardianAddress(guardianId) {
        const client = await pool.connect();
        try {
            const res = await client.query('SELECT a.street, a.city, a.province, a.postal_code FROM addresses a JOIN guardians g USING(address_id) WHERE g.guardian_id=$1', [guardianId]);
    ...
    async updateGuardianAddress(guardianId, address) {
        const client = await pool.connect();
        try {
            const res = await client.query(`
                UPDATE addresses a 
    ...
    async addGuardianAddress(guardianId, address) {
        const client = await pool.connect();
        try {
...
exports.SchoolGuardiansRepository = SchoolGuardiansRepository;
