...
exports.SchoolClassesRepository = void 0;
...
class SchoolClassesRepository {
    ...
    async getClass(classId) {
        const client = await pool.connect();
        try {
            const res = await client.query('SELECT * FROM classes WHERE class_id = $1', [classId]);
            if (res.rowCount === 0) {
    ...
    async addClass(course) {
        ...
        try {
            const res = await client.query(`INSERT INTO classes(class_name, grade_level, capacity) VALUES($1, $2, $3)`, [course.className, course.gradeLevel, course.capacity]);
        }
        catch (err) {
    ...
    async updateClass(classId, updatedClass) {
        const client = await pool.connect();
        try {
    ...
    async deleteClass(classId) {
        const client = await pool.connect();
        try {
            const res = await client.query('DELETE FROM classes WHERE class_id = $1', [classId]);
            if (res.rowCount === 0) {
    ...
    async assignTeacherToClass(classId, teacherId) {
        const client = await pool.connect();
        try {
            const res = await client.query('UPDATE classes SET teacher_id = $1 WHERE class_id = $2', [teacherId, classId]);
            if (res.rowCount === 0) {
    ...
    async removeTeacherFromClass(classId) {
        const client = await pool.connect();
        try {
            const res = await client.query('UPDATE classes SET teacher_id = NULL WHERE class_id = $1', [classId]);
            if (res.rowCount === 0) {
    ...
    async getClassAvailableStudents(classId) {
        const client = await pool.connect();
        try {
            ...
            const enrolledStudentIds = res.rows.map(row => row.student_id);
...
exports.SchoolClassesRepository = SchoolClassesRepository;
